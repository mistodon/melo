...
pub fn generate_abc(
    ...
    for (index, piece) in pieces.iter().enumerate() {
        ...
        // TODO(claire): We need to escape `\"` to `"` before writing title/composer/voice name
        ...
        if let Some(composer) = piece.composer {
            writeln!(buffer, "C:{}", composer).map_err(|e| fmt_err(e, Some(source_map.clone())))?;
        }
        ...
        for voice in &piece.voices {
            // TODO(claire): Voice names with spaces only take the first word. Needs quotes?
